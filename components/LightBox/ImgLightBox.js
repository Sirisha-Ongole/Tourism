...
export default function Modal({ imgSrc, setShowModal,selectedType, images ,autoSlide = false, autoSlideInterval = 3000}) {
      ...
      const filterd = selectedType == "All" ? images : images.filter(image => image.type.includes(selectedType));
          useEffect(() => {
            setFilteredImages(filterd);
          }, [setFilteredImages]);
                  ...
                  <div className="col-span-1 text-end text-white place-self-end px-2">
                    ...
                    <div className="p-2">
                      ...
                      <div className="p-2 text-start">
                        <ShareDropDown currentImg={imgSrc}/>
                      </div>
                ...
                <TransformWrapper ref={Component.transformComponentRef}>
                  {(utils) => (
                    <React.Fragment>
                      <Controls {...utils} />
                      <TransformComponent>
                        <img className="lg:h-96 h-100 object-cover" src={`/images/${filteredImages[currentSlide].image}`} ref={refImage} alt="gallery" /> 
